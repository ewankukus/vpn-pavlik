...
certbot certonly --webroot \
    ...
    || error "Не удалось получить сертификат для $REALITY_DOMAIN — убедитесь что DNS указывает на этот сервер"
...
certbot certonly --webroot \
    ...
    --email "$LE_EMAIL" \
    --agree-tos --non-interactive --quiet \
    || error "Не удалось получить сертификат для $PANEL_DOMAIN — убедитесь что DNS указывает на этот сервер"
...
# Ждём появления БД (до 60 сек)
for i in {1..30}; do
    [[ -f "$X_UI_DB" ]] && break
    sleep 2
done
[[ ! -f "$X_UI_DB" ]] && error "БД 3X-UI не найдена: $X_UI_DB"

# Применяем настройки через x-ui CLI
x-ui setting -username "$PANEL_USER" -password "$PANEL_PASS" -port "$PANEL_PORT" 2>/dev/null || true

# Устанавливаем web base path
sqlite3 "$X_UI_DB" \
    "UPDATE settings SET value='$PANEL_PATH' WHERE key='webBasePath';" 2>/dev/null || true
...
# ================================================================
# Шаг 9: Генерация ключей Reality
# ================================================================
step "9 — Генерация ключей Reality"

XRAY_BIN=$(find /usr/local/x-ui/ /usr/bin/ /usr/local/bin/ -name "xray" 2>/dev/null | head -1 || true)
[[ -z "$XRAY_BIN" ]] && XRAY_BIN="/usr/local/x-ui/bin/xray"
[[ ! -f "$XRAY_BIN" ]] && error "Xray бинарник не найден. Убедитесь что 3X-UI установлен."
...
[[ -z "$PRIVATE_KEY" ]] && error "Не удалось сгенерировать ключи Reality"
...
if echo "$LOGIN_RESP" | grep -q '"success":true'; then
    ...
    VLESS_STREAM_SETTINGS=$(cat << JSONEOF
...
JSONEOF
    ...
    ADD_RESP=$(curl -s -b /tmp/x-ui-cookie.txt \
        -X POST "$PANEL_API/panel/inbound/add" \
        -H "Content-Type: application/json" \
        -d "{
          \"up\":0,\"down\":0,\"total\":0,
          \"remark\":\"VLESS-Reality-Frankfurt\",
          \"enable\":true,\"expiryTime\":0,
          \"listen\":\"\",\"port\":$XRAY_PORT,
          \"protocol\":\"vless\",
          \"settings\":$(echo "$VLESS_SETTINGS" | jq -Rs .),
          \"streamSettings\":$(echo "$VLESS_STREAM_SETTINGS" | jq -Rs .),
    ...
    if echo "$ADD_RESP" | grep -q '"success":true'; then
        ...
        warn "Не удалось создать inbound через API — настройте вручную в панели"
        CLIENT_UUID=""
    fi
else
...
TROJAN_KEYS=$("$XRAY_BIN" x25519 2>/dev/null)
TROJAN_PRIV=$(echo "$TROJAN_KEYS" | grep "Private key:" | awk '{print $3}')
TROJAN_PUB=$(echo "$TROJAN_KEYS"  | grep "Public key:"  | awk '{print $3}')
TROJAN_SHORT_ID=$(openssl rand -hex 8)

if echo "$LOGIN_RESP" | grep -q '"success":true'; then
    TROJAN_STREAM_SETTINGS=$(cat << JSONEOF
...
JSONEOF
    ...
    TROJAN_RESP=$(curl -s -b /tmp/x-ui-cookie.txt \
        -X POST "$PANEL_API/panel/inbound/add" \
        -H "Content-Type: application/json" \
        -d "{
          \"up\":0,\"down\":0,\"total\":0,
          \"remark\":\"Trojan-Reality-Frankfurt\",
          \"enable\":true,\"expiryTime\":0,
          \"listen\":\"\",\"port\":$TROJAN_PORT,
          \"protocol\":\"trojan\",
          \"settings\":$(echo "$TROJAN_SETTINGS" | jq -Rs .),
          \"streamSettings\":$(echo "$TROJAN_STREAM_SETTINGS" | jq -Rs .),
          \"sniffing\":\"{\\\"enabled\\\":true,\\\"destOverride\\\":[\\\"http\\\",\\\"tls\\\",\\\"quic\\\",\\\"fakedns\\\"]}\"
        }" 2>/dev/null || echo "{}")

    if echo "$TROJAN_RESP" | grep -q '"success":true'; then
...
# ================================================================
# Шаг 12: Настройка Nginx (два домена)
# ================================================================
...
# HTTPS — сайт-камуфляж (self-steal для Reality)
server {
...
server {
    ...
    # Блокируем всё кроме секретного пути
    location / {
        return 404;
    }
...
if ! systemctl is-active --quiet x-ui; then
    echo "[$TS] x-ui не запущен — перезапускаю" >> "$LOG"
    systemctl restart x-ui
fi

if ! systemctl is-active --quiet nginx; then
    echo "[$TS] nginx не запущен — перезапускаю" >> "$LOG"
...
ls -t "\$BDIR"/vpn_backup_*.tar.gz 2>/dev/null | tail -n +8 | xargs rm -f 2>/dev/null || true
...
echo ""
echo -e "${YELLOW}Параметры сохранены в /root/vpn-install-info.txt${NC}"
echo ""
